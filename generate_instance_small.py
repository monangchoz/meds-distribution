...
# Load small product codes
def get_small_product_codes() -> set:
    ...
    small_threshold = np.quantile(volumes, 0.33)

    return {code for code, volume in product_data if volume <= small_threshold}


SMALL_PRODUCT_CODES = get_small_product_codes()


def get_customer_items_if_small_only(cust_id: str) -> List[Item]:
    ...
    try:
        ...
        if not all(item["PRODUCT_CODE"] in SMALL_PRODUCT_CODES for item in raw_item_dict_list):
            return []  # Skip if any product is not small
...
def generate_customers(cabang, num_customers) -> List[Customer]:
    ...
    for customer_dict in customers_dict_list:
        ...
        items = get_customer_items_if_small_only(str(cust_id))
        if items:
