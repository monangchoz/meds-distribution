    ...
    for folder_name, algorithm_name in algorithm_folders.items():
        ...
        for file_path in csv_files:
            ...
            try:
                ...
                print(f"✓ Processed {filename} -> customers: {num_customers}→{round_customer_count(num_customers)}")
...
def create_combined_cluster_analysis(df):
    """
    Create combined multipanel plot: 
    Left Panel - Performance by cluster count
    Right Panel - Overall average performance by algorithm (averaged across all clusters)
    """
    ...
    df_clean = df.dropna(subset=['num_clusters', 'algorithm', 'total_cost']).copy()
    
    if df_clean.empty:
        print("No clean data available")
        return None
    
    print(f"📊 Creating combined cluster analysis:")
    ...
    # Create figure with 2 subplots (side by side) with more spacing
    ...
    # Set colors to match the original plot
    algorithm_colors = {
        'BRKGA': '#1f77b4',  # Blue
        'DE': '#d62728',     # Red  
        'GA': '#e377c2',     # Pink
    ...
    # === LEFT PANEL: ALGORITHM PERFORMANCE ACROSS CLUSTER COUNTS ===
    cluster_analysis = df_clean.groupby(['algorithm', 'num_clusters']).agg({
        'total_cost': 'mean'
    }).reset_index()
    
    cluster_analysis.columns = ['algorithm', 'num_clusters', 'avg_cost']
    
    # Set up bar chart parameters for left panel
    ...
    for i, alg in enumerate(algorithms):
        ...
        for cluster in clusters:
            ...
            if not cluster_data.empty:
                costs.append(cluster_data['avg_cost'].iloc[0])
            else:
                costs.append(0)
        
        offset = (i - len(algorithms)/2 + 0.5) * width
        color = algorithm_colors.get(alg, f'C{i}')
        ax1.bar(x1 + offset, costs, width, label=alg, color=color, alpha=0.8)
    ...
    # === RIGHT PANEL: AVERAGE COST BY CLUSTER COUNT (ACROSS ALL ALGORITHMS) ===
    # Calculate average cost per cluster count (averaged across all algorithms)
    cluster_avg = df_clean.groupby('num_clusters').agg({
        'total_cost': 'mean'
    }).reset_index()
    
    cluster_avg.columns = ['num_clusters', 'avg_cost']
    cluster_avg = cluster_avg.sort_values('num_clusters')  # Sort by cluster count
    ...
    # Use a gradient color scheme for clusters
    cluster_colors = ['#2E7D32', '#43A047', '#66BB6A', '#81C784', '#A5D6A7']  # Green gradient
    colors = cluster_colors[:len(clusters)]
    costs = [cluster_avg[cluster_avg['num_clusters'] == cluster]['avg_cost'].iloc[0] for cluster in clusters]
    
    bars = ax2.bar(x2, costs, bar_width, color=colors, alpha=0.8)
    
    # Customize RIGHT PANEL
    ax2.set_title('Average Cost by Cluster Count\n(Averaged Across All Algorithms)', 
                 fontweight='bold', fontsize=14)
    ...
    # Main title
    plt.suptitle('Algorithm Performance Analysis: Cluster-Specific vs Cluster Average', 
                fontweight='bold', fontsize=16, y=0.98)
    
    # Adjust layout with more horizontal spacing and better margins
    plt.tight_layout(rect=[0.05, 0, 0.95, 0.95], w_pad=3.0)
    
    return fig

def create_summary_comparison(df):
    """
    Create summary comparison showing the difference between cluster-specific and overall performance
    """
    if df.empty:
        return None
    ...
    # Overall averages
    overall_avg = df_clean.groupby('algorithm')['total_cost'].mean()
    ...
    for alg in algorithms:
        ...
        # Overall average
        overall = alg_data['total_cost'].mean()
        ...
        for cluster in clusters:
            cluster_data = alg_data[alg_data['num_clusters'] == cluster]
            if not cluster_data.empty:
                cluster_avgs[cluster] = cluster_data['total_cost'].mean()
            else:
        ...
        row = [
            alg,
            f"{overall:,.0f}",
            f"{cluster_avgs.get(1, 0):,.0f}" if cluster_avgs.get(1, 0) > 0 else "N/A",
            f"{cluster_avgs.get(3, 0):,.0f}" if cluster_avgs.get(3, 0) > 0 else "N/A",
            f"{cluster_avgs.get(5, 0):,.0f}" if cluster_avgs.get(5, 0) > 0 else "N/A",
            f"{std_dev:,.0f}",
            consistency
        ]
    ...
    plt.title('Performance Comparison: Cluster-Specific vs Overall Average Cost', 
...
def main():
    ...
    print("🔍 Combined Analysis: Cluster Performance vs Cluster Average")
    print("=" * 70)
    print("Creating multipanel visualization:")
    print("  - Left Panel: Performance by cluster count (algorithm comparison)")
    print("  - Right Panel: Average cost by cluster count (across all algorithms)")
    print("=" * 70)
    ...
    fig1 = create_combined_cluster_analysis(df)
    if fig1:
        plt.show()
    
    print("\n📋 Creating performance comparison table...")
    fig2 = create_summary_comparison(df)
    if fig2:
        plt.show()
    
    # Print quick insights
    df_clean = df.dropna(subset=['algorithm', 'total_cost'])
    if not df_clean.empty:
        print("\n" + "=" * 70)
        print("📊 QUICK INSIGHTS")
        print("=" * 70)
        
        # Overall ranking by algorithm
        overall_ranking = df_clean.groupby('algorithm')['total_cost'].mean().sort_values()
        print("🏆 Algorithm Ranking (averaged across all clusters):")
        for i, (alg, cost) in enumerate(overall_ranking.items(), 1):
            print(f"  {i}. {alg}: {cost:,.0f} average cost")
        
        # Cluster impact analysis
        cluster_ranking = df_clean.groupby('num_clusters')['total_cost'].mean().sort_values()
        print(f"\n📈 Cluster Count Impact (averaged across all algorithms):")
        for cluster, cost in cluster_ranking.items():
            print(f"  {int(cluster)} cluster{'s' if cluster != 1 else ''}: {cost:,.0f} average cost")
    ...
    if save_plots == 'y':
        if fig1:
            fig1.savefig('combined_cluster_vs_overall_analysis.png', dpi=300, bbox_inches='tight')
            print("✅ Saved combined_cluster_vs_overall_analysis.png")
        
        if fig2:
            fig2.savefig('performance_comparison_table.png', dpi=300, bbox_inches='tight')
            print("✅ Saved performance_comparison_table.png")
