...
# Load medium product codes
def get_medium_product_codes() -> set:
...
MEDIUM_PRODUCT_CODES = get_medium_product_codes()


def get_customer_items_if_medium_only(cust_id: str) -> List[Item]:
    ...
    try:
        ...
        if not all(item["PRODUCT_CODE"] in MEDIUM_PRODUCT_CODES for item in raw_item_dict_list):
            return []  # Skip if any product is not medium
...
def generate_customers(cabang, num_customers) -> List[Customer]:
    ...
    for customer_dict in customers_dict_list:
        ...
        items = get_customer_items_if_medium_only(str(cust_id))
        if items:
