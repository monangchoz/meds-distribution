...
# Load large product codes
def get_large_product_codes() -> set:
    ...
    large_threshold = np.quantile(volumes, 0.66)

    return {code for code, volume in product_data if volume > large_threshold}


LARGE_PRODUCT_CODES = get_large_product_codes()


def get_customer_items_if_large_only(cust_id: str) -> List[Item]:
    ...
    try:
        ...
        if not all(item["PRODUCT_CODE"] in LARGE_PRODUCT_CODES for item in raw_item_dict_list):
            return []  # Skip if any product is not large
...
def generate_customers(cabang, num_customers) -> List[Customer]:
    ...
    for customer_dict in customers_dict_list:
        ...
        items = get_customer_items_if_large_only(str(cust_id))
        if items:
